...
if ($receipt_id <= 0) {
    ...
    $check_sql = "SELECT * FROM uric_acid WHERE receipt_id = $receipt_id";
    ...
    if (mysqli_num_rows($check_result) > 0) {
        ...
        $existing_data = ['uric_acid' => '', 'reporting_datetime' => ''];
    }
}

if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $uric_acid = mysqli_real_escape_string($conn, $_POST['uric_acid']);

    if ($edit_mode) {
        $update_sql = "UPDATE uric_acid SET uric_acid = '$uric_acid', reporting_datetime = NOW() WHERE receipt_id = $receipt_id";
        ...
        $insert_sql = "INSERT INTO uric_acid (receipt_id, uric_acid, reporting_datetime) VALUES ($receipt_id, '$uric_acid', NOW())";
...
<title>Plasma Glucose (Fasting) Test Form</title>
<style>
        body {
...
<div class="container" style="margin-bottom: 100px;">
    <h2><?= $edit_mode ? "Edit Uric Acid Test Result" : "Enter Uric Acid Test Result" ?></h2>
    ...
    <form method="post" action="">
        <label for="uric_acid">Uric Acid Result:</label>
        <input type="text" name="uric_acid" id="uric_acid" required value="<?= htmlspecialchars($existing_data['uric_acid']) ?>" />
