...
if ($receipt_id <= 0) {
    ...
    $check_sql = "SELECT * FROM lipase WHERE receipt_id = $receipt_id";
    ...
    if (mysqli_num_rows($check_result) > 0) {
        ...
        $existing_data = ['lipase' => '', 'reporting_datetime' => ''];
    }
}

if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $lipase = mysqli_real_escape_string($conn, $_POST['lipase']);

    if ($edit_mode) {
        $update_sql = "UPDATE lipase SET lipase = '$lipase', reporting_datetime = NOW() WHERE receipt_id = $receipt_id";
        ...
        $insert_sql = "INSERT INTO lipase (receipt_id, lipase, reporting_datetime) VALUES ($receipt_id, '$lipase', NOW())";
...
<div class="container" style="margin-bottom: 100px;">
    ...
    <form method="post" action="">
        <label for="lipase">Lipase Result:</label>
        <input type="text" name="lipase" id="lipase" required value="<?= htmlspecialchars($existing_data['lipase']) ?>" />
