...
if ($receipt_id <= 0) {
    ...
    $check_sql = "SELECT * FROM crp WHERE receipt_id = $receipt_id";
    ...
    if (mysqli_num_rows($check_result) > 0) {
        ...
        $existing_data = ['crp' => '', 'reporting_datetime' => ''];
    }
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($edit_mode) {
        $update_sql = "UPDATE crp SET crp = '$crp', reporting_datetime = NOW() WHERE receipt_id = $receipt_id";
        ...
        $insert_sql = "INSERT INTO crp (receipt_id, crp, reporting_datetime) VALUES ($receipt_id, '$crp', NOW())";
...
<div class="container" style="margin-bottom: 100px;">
    <h2><?= $edit_mode ? "Edit CRP Test Result" : "Enter CRP Test Result" ?></h2>
    ...
    <form method="post" action="">
        <label for="crp">Serum CRP:</label>
        <input type="text" name="crp" id="crp" required value="<?= htmlspecialchars($existing_data['crp']) ?>" />
