...
if ($receipt_id <= 0) {
    ...
    $check_sql = "SELECT * FROM btct WHERE receipt_id = $receipt_id";
    ...
    if (mysqli_num_rows($check_result) > 0) {
        ...
        $existing_data = ['bt' => '', 'ct' => '', 'reporting_datetime' => ''];
    }
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($edit_mode) {
        $update_sql = "UPDATE btct SET bt = '$bt', ct = '$ct', reporting_datetime = NOW() WHERE receipt_id = $receipt_id";
        ...
        $insert_sql = "INSERT INTO btct (receipt_id, bt, ct, reporting_datetime) VALUES ($receipt_id, '$bt', '$ct', NOW())";
...
<div class="container" style="margin-bottom: 100px;">
    ...
    <form method="post" action="">
        <label for="bt">BT Result:</label>
        <input type="text" name="bt" id="bt" required value="<?= htmlspecialchars($existing_data['bt']) ?>" />

        <label for="ct">CT Result:</label>
        <input type="text" name="ct" id="ct" value="<?= htmlspecialchars($existing_data['ct']) ?>" />
