...
if ($receipt_id <= 0) {
    ...
    $check_sql = "SELECT * FROM dengue_ns1 WHERE receipt_id = $receipt_id";
    ...
    if (mysqli_num_rows($check_result) > 0) {
        ...
        $existing_data = ['dengue_ns1' => '', 'reporting_datetime' => ''];
    }
}

if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $dengue_ns1 = mysqli_real_escape_string($conn, $_POST['dengue_ns1']);

    if ($edit_mode) {
        $update_sql = "UPDATE dengue_ns1 SET dengue_ns1 = '$dengue_ns1', reporting_datetime = NOW() WHERE receipt_id = $receipt_id";
        ...
        $insert_sql = "INSERT INTO dengue_ns1 (receipt_id, dengue_ns1, reporting_datetime) VALUES ($receipt_id, '$dengue_ns1', NOW())";
...
<title>DENGUE NS 1 Test Form</title>
<style>
        body {
...
<div class="container" style="margin-bottom: 100px;">
    ...
    <form method="post" action="">
        <label for="dengue_ns1">Dengue Ns1 Result:</label>
        <input type="text" name="dengue_ns1" id="dengue_ns1" required value="<?= htmlspecialchars($existing_data['dengue_ns1']) ?>" />
