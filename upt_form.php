...
if ($receipt_id <= 0) {
    ...
    $check_sql = "SELECT * FROM upt WHERE receipt_id = $receipt_id";
    ...
    if (mysqli_num_rows($check_result) > 0) {
        ...
        $existing_data = ['upt' => '', 'reporting_datetime' => ''];
    }
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($edit_mode) {
        $update_sql = "UPDATE upt SET upt = '$upt', reporting_datetime = NOW() WHERE receipt_id = $receipt_id";
        ...
        $insert_sql = "INSERT INTO upt (receipt_id, upt, reporting_datetime) VALUES ($receipt_id, '$upt', NOW())";
...
<div class="container" style="margin-bottom: 100px;">
    ...
    <form method="post" action="">
        <label for="upt">UPT Result:</label>
        <input type="text" name="upt" id="upt" required value="<?= htmlspecialchars($existing_data['upt']) ?>" />
