...
if ($receipt_id <= 0) {
    ...
    $check_receipt_sql = "SELECT * FROM serum_prolactin WHERE receipt_id = '$receipt_id'";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // Get data from form
    $serum_prolactin = $_POST['serum_prolactin'];
    

    if ($edit_mode) {
        // Update existing record
        $update_sql = "UPDATE serum_prolactin SET
                        serum_prolactin = '$serum_prolactin',
        ...
        // Insert new record
        $insert_sql = "INSERT INTO serum_prolactin (
                        receipt_id, serum_prolactin, reporting_datetime
                    ) VALUES (
                        '$receipt_id', '$serum_prolactin', NOW()
                    )";
...
<head>
        <meta charset="UTF-8">
    <title>Serum Prolactin Test Result</title>
...
<div class="container" style="margin-bottom: 100px;">
    <h2><?= $edit_mode ? "Edit Serum Prolactin Test Result" : "Enter Serum Prolactin Test Result" ?></h2>
    ...
    <form method="post" action="">
        <label for="serum_prolactin">Serum Prolactin:</label>
        <input type="text" name="serum_prolactin" id="serum_prolactin" required value="<?= htmlspecialchars($existing_data['serum_prolactin']) ?>" />
