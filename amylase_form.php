...
if ($receipt_id <= 0) {
    ...
    $check_sql = "SELECT * FROM amylase WHERE receipt_id = $receipt_id";
    ...
    if (mysqli_num_rows($check_result) > 0) {
        ...
        $existing_data = ['amylase' => '', 'reporting_datetime' => ''];
    }
}

if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $amylase = mysqli_real_escape_string($conn, $_POST['amylase']);

    if ($edit_mode) {
        $update_sql = "UPDATE amylase SET amylase = '$amylase', reporting_datetime = NOW() WHERE receipt_id = $receipt_id";
        ...
        $insert_sql = "INSERT INTO amylase (receipt_id, amylase, reporting_datetime) VALUES ($receipt_id, '$amylase', NOW())";
...
<div class="container" style="margin-bottom: 100px;">
    ...
    <form method="post" action="">
        <label for="amylase">Amylase Result:</label>
        <input type="text" name="amylase" id="amylase" required value="<?= htmlspecialchars($existing_data['amylase']) ?>" />
