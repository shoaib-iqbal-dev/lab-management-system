...
if ($receipt_id <= 0) {
    ...
    $check_sql = "SELECT * FROM asot WHERE receipt_id = $receipt_id";
    ...
    if (mysqli_num_rows($check_result) > 0) {
        ...
        $existing_data = ['asot' => '', 'reporting_datetime' => ''];
    }
}

if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $asot = mysqli_real_escape_string($conn, $_POST['asot']);

    if ($edit_mode) {
        $update_sql = "UPDATE asot SET asot = '$asot', reporting_datetime = NOW() WHERE receipt_id = $receipt_id";
        ...
        $insert_sql = "INSERT INTO asot (receipt_id, asot, reporting_datetime) VALUES ($receipt_id, '$asot', NOW())";
...
<div class="container" style="margin-bottom: 100px;">
    ...
    <form method="post" action="">
        <label for="asot">ASOT Result:</label>
        <input type="text" name="asot" id="asot" required value="<?= htmlspecialchars($existing_data['asot']) ?>" />
