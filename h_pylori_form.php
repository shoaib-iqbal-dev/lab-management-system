...
if ($receipt_id <= 0) {
    ...
    $check_sql = "SELECT * FROM h_pylori WHERE receipt_id = $receipt_id";
    ...
    if (mysqli_num_rows($check_result) > 0) {
        ...
        $existing_data = ['h_pylori' => '', 'reporting_datetime' => ''];
    }
}

if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $h_pylori = mysqli_real_escape_string($conn, $_POST['h_pylori']);

    if ($edit_mode) {
        $update_sql = "UPDATE h_pylori SET h_pylori = '$h_pylori', reporting_datetime = NOW() WHERE receipt_id = $receipt_id";
        ...
        $insert_sql = "INSERT INTO h_pylori (receipt_id, h_pylori, reporting_datetime) VALUES ($receipt_id, '$h_pylori', NOW())";
...
<title>H PYLORI Test Form</title>
<style>
        body {
...
<div class="container" style="margin-bottom: 100px;">
    ...
    <form method="post" action="">
        <label for="h_pylori">H Pylori Result:</label>
        <input type="text" name="h_pylori" id="h_pylori" required value="<?= htmlspecialchars($existing_data['h_pylori']) ?>" />
