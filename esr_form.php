...
if ($receipt_id <= 0) {
    ...
    $check_sql = "SELECT * FROM esr WHERE receipt_id = $receipt_id";
    ...
    if (mysqli_num_rows($check_result) > 0) {
        ...
        $existing_data = ['esr' => '', 'reporting_datetime' => ''];
    }
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($edit_mode) {
        $update_sql = "UPDATE esr SET esr = '$esr', reporting_datetime = NOW() WHERE receipt_id = $receipt_id";
        ...
        $insert_sql = "INSERT INTO esr (receipt_id, esr, reporting_datetime) VALUES ($receipt_id, '$esr', NOW())";
...
<div class="container" style="margin-bottom: 100px;">
    ...
    <form method="post" action="">
        <label for="esr">ESR Result:</label>
        <input type="text" name="esr" id="esr" required value="<?= htmlspecialchars($existing_data['esr']) ?>" />
