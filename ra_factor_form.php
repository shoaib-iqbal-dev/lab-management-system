...
if ($receipt_id <= 0) {
    ...
    $check_sql = "SELECT * FROM ra_factor WHERE receipt_id = $receipt_id";
    ...
    if (mysqli_num_rows($check_result) > 0) {
        ...
        $existing_data = ['ra_factor' => '', 'reporting_datetime' => ''];
    }
}

if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $ra_factor = mysqli_real_escape_string($conn, $_POST['ra_factor']);

    if ($edit_mode) {
        $update_sql = "UPDATE ra_factor SET ra_factor = '$ra_factor', reporting_datetime = NOW() WHERE receipt_id = $receipt_id";
        ...
        $insert_sql = "INSERT INTO ra_factor (receipt_id, ra_factor, reporting_datetime) VALUES ($receipt_id, '$ra_factor', NOW())";
...
<title>RA FACTOR Test Form</title>
<style>
        body {
...
<div class="container" style="margin-bottom: 100px;">
    ...
    <form method="post" action="">
        <label for="ra_factor">RA Factor Result:</label>
        <input type="text" name="ra_factor" id="ra_factor" required value="<?= htmlspecialchars($existing_data['ra_factor']) ?>" />
