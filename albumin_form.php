...
if ($receipt_id <= 0) {
    ...
    $check_sql = "SELECT * FROM albumin WHERE receipt_id = $receipt_id";
    ...
    if (mysqli_num_rows($check_result) > 0) {
        ...
        $existing_data = ['albumin' => '', 'reporting_datetime' => ''];
    }
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($edit_mode) {
        $update_sql = "UPDATE albumin SET albumin = '$albumin', reporting_datetime = NOW() WHERE receipt_id = $receipt_id";
        ...
        $insert_sql = "INSERT INTO albumin (receipt_id, albumin, reporting_datetime) VALUES ($receipt_id, '$albumin', NOW())";
...
<div class="container" style="margin-bottom: 100px;">
    ...
    <form method="post" action="">
        <label for="albumin">Albumin Result:</label>
        <input type="text" name="albumin" id="albumin" required value="<?= htmlspecialchars($existing_data['albumin']) ?>" />
