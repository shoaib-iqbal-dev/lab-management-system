...
// Validate input
if (empty($receipt_id) || empty($mr_no) || strtolower($test_name) !== 'serum amh') {
...
$testQuery = "SELECT serum_amh.*, receipts.receipt_no, receipts.actual_fee, receipts.discount, receipts.total_fee, receipts.payment_date
              FROM serum_amh
              LEFT JOIN receipts ON serum_amh.receipt_id = receipts.receipt_id
              WHERE serum_amh.receipt_id = ?";
$testStmt = $conn->prepare($testQuery);
$testStmt->bind_param("i", $receipt_id);
$testStmt->execute();
$testResult = $testStmt->get_result();
if ($testResult->num_rows === 0) {
    echo "<h3>No SERUM AMH result found for this receipt.</h3>";
    exit;
}
...
<p style="font-size:10px;">Reference Values:<br><p>
