...
// Validate input
if (empty($receipt_id) || empty($mr_no) || strtolower($test_name) !== 'triglycerides') {
...
$testQuery = "SELECT triglycerides.*, receipts.receipt_no, receipts.actual_fee, receipts.discount, receipts.total_fee, receipts.payment_date
              FROM triglycerides
              LEFT JOIN receipts ON triglycerides.receipt_id = receipts.receipt_id
              WHERE triglycerides.receipt_id = ?";
$testStmt = $conn->prepare($testQuery);
$testStmt->bind_param("i", $receipt_id);
$testStmt->execute();
$testResult = $testStmt->get_result();
if ($testResult->num_rows === 0) {
    echo "<h3>No Triglycerides result found for this receipt.</h3>";
    exit;
}
