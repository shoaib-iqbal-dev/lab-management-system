...
// Validate input
if (empty($receipt_id) || empty($mr_no) || strtolower($test_name) !== 'cholesterol') {
...
$testQuery = "SELECT cholesterol.*, receipts.receipt_no, receipts.actual_fee, receipts.discount, receipts.total_fee, receipts.payment_date
              FROM cholesterol
              LEFT JOIN receipts ON cholesterol.receipt_id = receipts.receipt_id
              WHERE cholesterol.receipt_id = ?";
$testStmt = $conn->prepare($testQuery);
$testStmt->bind_param("i", $receipt_id);
$testStmt->execute();
$testResult = $testStmt->get_result();
if ($testResult->num_rows === 0) {
    echo "<h3>No TG Cholesterol result found for this receipt.</h3>";
    exit;
}
...
<html>
<head>
    <title>CHOLESTEROL Report</title>
    <style>
        body {
