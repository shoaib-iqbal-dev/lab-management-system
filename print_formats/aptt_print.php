...
// Validate input
if (empty($receipt_id) || empty($mr_no) || strtolower($test_name) !== 'aptt') {
...
$testQuery = "SELECT aptt.*, receipts.receipt_no, receipts.actual_fee, receipts.discount, receipts.total_fee, receipts.payment_date
              FROM aptt
              LEFT JOIN receipts ON aptt.receipt_id = receipts.receipt_id
              WHERE aptt.receipt_id = ?";
$testStmt = $conn->prepare($testQuery);
$testStmt->bind_param("i", $receipt_id);
$testStmt->execute();
$testResult = $testStmt->get_result();
if ($testResult->num_rows === 0) {
    echo "<h3>No APTT result found for this receipt.</h3>";
    exit;
}
...
<hr style="margin-top: 0px;margin-bottom: 0px;">
<p style="margin-bottom: 0px;margin-top: 01px;"><b>Coagulation Profile</b></p>
