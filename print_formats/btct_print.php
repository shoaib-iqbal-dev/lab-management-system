...
// Validate input
if (empty($receipt_id) || empty($mr_no) || strtolower($test_name) !== 'btct') {
...
$testQuery = "SELECT btct.*, receipts.receipt_no, receipts.actual_fee, receipts.discount, receipts.total_fee, receipts.payment_date
              FROM btct
              LEFT JOIN receipts ON btct.receipt_id = receipts.receipt_id
              WHERE btct.receipt_id = ?";
$testStmt = $conn->prepare($testQuery);
$testStmt->bind_param("i", $receipt_id);
$testStmt->execute();
$testResult = $testStmt->get_result();
if ($testResult->num_rows === 0) {
    echo "<h3>No BTCT result found for this receipt.</h3>";
    exit;
}
...
<table width="100%">
  <tr>
	...
	<h3 style="margin-bottom: 0px;">Department of Haematology</h3>
	</td>
...
<tr>
    ...
    <tr>
	<td>Bleeding Time</td>
	<td>Adults: 2 - 11 Minutes<br>Children: 1 - 13 	Minutes</td>
	<td><?= $row['bt'] ?></td>
	<td></td>
	</tr>
    <tr>
