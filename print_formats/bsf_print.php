...
// Validate input
if (empty($receipt_id) || empty($mr_no) || strtolower($test_name) !== 'bsf') {
...
$testQuery = "SELECT bsf.*, receipts.receipt_no, receipts.actual_fee, receipts.discount, receipts.total_fee, receipts.payment_date
              FROM bsf
              LEFT JOIN receipts ON bsf.receipt_id = receipts.receipt_id
              WHERE bsf.receipt_id = ?";
$testStmt = $conn->prepare($testQuery);
$testStmt->bind_param("i", $receipt_id);
$testStmt->execute();
$testResult = $testStmt->get_result();
if ($testResult->num_rows === 0) {
    echo "<h3>No BSF result found for this receipt.</h3>";
    exit;
}
...
<table width="100%">
  <tr>
    ...
    <td style="font-size:12px; width: 35%;"><b>Collection DateTime:</b> <?= date('d-m-Y h:i A', strtotime($row['payment_date'])) ?> </td>
  </tr>
  <tr>
    <td style="font-size:12px; width: 35%;"><b>Reporting DateTime:</b> <?= date('d-m-Y h:i A', strtotime($row['reporting_datetime'])) ?></td>
  </tr>
</table>
<hr style="margin-top: 0px;margin-bottom: 0px;">
<table class="result-table" style="margin-top: 3px; margin-bottom: 10px; font-size:12px; width: 100%; border-collapse: collapse;">
  <tr>
    <th style="width: 37%; border: none; vertical-align: top; text-align: left;padding-right: 50px;">Plasma Glucose (Fasting)<br><br><p style="font-size:7px; font-weight: normal;"></p></th>
    <th style="border: none;width: 35%;">
      <div class="chart-wrapper" id="bsfChart">
  ...
  <?= $row['bsf'] ?><sub style="font-size: 16px; vertical-align: sub;">mg/dL</sub>
	  </div><div style="font-size: 18px; font-weight: bold; margin-left: 40px;">
<?php
$value = trim($row['bsf']);

// If value is empty, 0, "-", or not numeric, show nothing
if ($value === '' || $value === '0' || $value === '-' || !is_numeric($value)) {
    echo '';
} else {
    $value = (float)$value; // Convert to number safely
    if ($value < 60) {
        echo 'Low';
    } elseif ($value <= 110) {
        echo 'Normal';
...
?>
  ...
  <tr>
    ...
    <td colspan="2" style="border: none; font-size: 10px;">
      <b>• Low (&lt;60) • Normal (60 - 110) • High (&gt;110)</b>
    </td>
  </tr>
</table>
<hr style="margin-top: 0px;margin-bottom: 0px;">
<table class="result-table" style="margin-top: 3px; margin-bottom: 10px; font-size:12px; width: 100%; border-collapse: collapse;">
  <tr>
    <th style="width: 37%; border: none; vertical-align: top; text-align: left;padding-right: 50px;">Plasma Glucose (Random)<br><br><p style="font-size:7px; font-weight: normal;"></p></th>
    <th style="border: none;"width: 35%;">
      <div class="chart-wrapper" id="bsrChart">
        <div class="y-scale" style="height: 110px;top: -5px;">
          <div>230</div>
          <div>200</div>
          <div>170</div>
		...
		<th style="width: 28%; border: none; text-align: left;">
	  <div style="font-size: 55px; line-height: 1; font-weight: 300; font-family: 'Arial', sans-serif;">
  <?= $row['bsr'] ?><sub style="font-size: 16px; vertical-align: sub;">mg/dL</sub>
	  </div><div style="font-size: 18px; font-weight: bold; margin-left: 40px;">
<?php
$value = trim($row['bsr']);
...
?>
  ...
  <tr>
    <td style="border: none;"></td>
    <td colspan="2" style="border: none; font-size: 10px;">
      <b>• Low (&lt;80) • Normal (80 - 180) • High (&gt;180)</b>
    </td>
  </tr>
</table>
...
const bsfValue = parseFloat("<?= $row['bsf'] ?>");
const bsfChart = document.getElementById('bsfChart');

const bsfMin = 50.0;
const bsfMax = 230.0;

if (!isNaN(bsfValue)) {
  let percentage = 100 - ((bsfValue - bsfMin) / (bsfMax - bsfMin)) * 100;
  ...
  marker.innerText = bsfValue.toFixed(1);

  bsfChart.appendChild(marker);
}


const bsrValue = parseFloat("<?= $row['bsr'] ?>");
const bsrChart = document.getElementById('bsrChart');
...
if (!isNaN(bsrValue)) {
  let percentage = 100 - ((bsrValue - bsrMin) / (bsrMax - bsrMin)) * 100;
  ...
  marker.className = 'marker';
  marker.style.top = `${percentage}%`;
  marker.innerText = bsrValue.toFixed(1);

  bsrChart.appendChild(marker);
}
