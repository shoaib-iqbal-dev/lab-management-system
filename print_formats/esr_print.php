...
// Validate input
if (empty($receipt_id) || empty($mr_no) || strtolower($test_name) !== 'esr') {
...
$testQuery = "SELECT esr.*, receipts.receipt_no, receipts.actual_fee, receipts.discount, receipts.total_fee, receipts.payment_date
              FROM esr
              LEFT JOIN receipts ON esr.receipt_id = receipts.receipt_id
              WHERE esr.receipt_id = ?";
$testStmt = $conn->prepare($testQuery);
$testStmt->bind_param("i", $receipt_id);
$testStmt->execute();
$testResult = $testStmt->get_result();
if ($testResult->num_rows === 0) {
    echo "<h3>No ESR result found for this receipt.</h3>";
    exit;
}
...
<tr>
    ...
    <tr>
	<td>ESR</td>
	<td>Male: 0 - 10 mm/hr<br>Female: 0 - 15 mm/hr</td>
	<td><?= $row['esr'] ?></td>
	<td></td>
	</tr>
