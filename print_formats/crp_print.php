...
// Validate input
if (empty($receipt_id) || empty($mr_no) || strtolower($test_name) !== 'crp') {
...
$testQuery = "SELECT crp.*, receipts.receipt_no, receipts.actual_fee, receipts.discount, receipts.total_fee, receipts.payment_date
              FROM crp
              LEFT JOIN receipts ON crp.receipt_id = receipts.receipt_id
              WHERE crp.receipt_id = ?";
$testStmt = $conn->prepare($testQuery);
$testStmt->bind_param("i", $receipt_id);
$testStmt->execute();
$testResult = $testStmt->get_result();
if ($testResult->num_rows === 0) {
    echo "<h3>No CRP result found for this receipt.</h3>";
    exit;
}
...
<p style="margin-bottom: 0px;margin-top: 0px;">Serum CRP</p>

<table class="result-table" style="margin-top: -15px; margin-bottom: 10px; font-size:12px; width: 100%; border-collapse: collapse;">
  <tr>
    <th style="width: 40%; border: none;">(Semi Quantitative)</th>
    <th style="border: none;"width: 30%;">
      <div class="chart-wrapper" id="crpChart">
        <div class="y-scale" style="height: 90px;top: 5px;">
          <div></div>
		  <div>6</div>
          <div>5</div>
          <div>4</div>
		...
		<th style="width: 30%; border: none; text-align: left;">
	  <div style="font-size: 55px; line-height: 1; font-weight: 300; font-family: 'Arial', sans-serif;">
  <?= $row['crp'] ?><sub style="font-size: 16px; vertical-align: sub;">mg/L</sub>
	  </div><div style="font-size: 18px; font-weight: bold; margin-left: 30px;">
    <?= ($row['crp'] <= 6) ? 'Normal' : 'High'; ?>
</div>
  ...
  <tr>
    ...
    <td style="border: none; font-size: 10px; text-align:right;">
      <b>• Normal (=6) &nbsp&nbsp• High (>6)</b>
    </td>
	<td style="border: none;"></td>
  </tr>
</table>
<p style="font-size:10px;">The Semi quatittative test can be performed in the same way as the quantitative test using dilutions of the serum in the saline.
</p>
...
<p style="font-size:12px;margin-top: 15px;margin-bottom: 0px;">
</p>
...
<script>
const crpValue = parseFloat("<?= $row['crp'] ?>");
const chart = document.getElementById('crpChart');

const min = 0;
const max = 7;

if (!isNaN(crpValue)) {
  let percentage = 100 - ((crpValue - min) / (max - min)) * 100;
  ...
  marker.innerText = crpValue.toFixed(1);
