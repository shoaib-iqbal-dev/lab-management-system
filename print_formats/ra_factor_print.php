...
// Validate input
if (empty($receipt_id) || empty($mr_no) || strtolower($test_name) !== 'ra factor') {
...
$testQuery = "SELECT ra_factor.*, receipts.receipt_no, receipts.actual_fee, receipts.discount, receipts.total_fee, receipts.payment_date
              FROM ra_factor
              LEFT JOIN receipts ON ra_factor.receipt_id = receipts.receipt_id
              WHERE ra_factor.receipt_id = ?";
$testStmt = $conn->prepare($testQuery);
$testStmt->bind_param("i", $receipt_id);
$testStmt->execute();
$testResult = $testStmt->get_result();
if ($testResult->num_rows === 0) {
    echo "<h3>No Blood Grouping result found for this receipt.</h3>";
    exit;
}
...
<hr style="margin-top: 0px;margin-bottom: 0px;border-top-width: 0px;border-top-style: solid;">
<table width="100%">
  <tr>
...
<table class="result-table" style="margin-top: -2px;margin-bottom: 30px; font-size:12">
    
<tr>
        <th style="width: 28%; border-left: none;">Test</th>
        <th style="width: 30%;">Reference Value</th>
		<th style="width: 15%;">Result</th>
		<th style="width: 22%; border-right: none;"></th>
    </tr>
    <tr>
	<td>R.A FACTOR (Semi Quantitative)</td>
	<td>0 -- 8 iu/ml</td>
	<td><?= $row['ra_factor'] ?></td>
	<td></td>
	</tr>
...
<p style="font-size:12;">The Semi quantitative test can be performed in the same way as the quantitative test <br>using dilutions of the serum in saline</p>
