...
// Validate input
if (empty($receipt_id) || empty($mr_no) || strtolower($test_name) !== 'hba1c') {
...
$testQuery = "SELECT hba1c.*, receipts.receipt_no, receipts.actual_fee, receipts.discount, receipts.total_fee, receipts.payment_date
              FROM hba1c
              LEFT JOIN receipts ON hba1c.receipt_id = receipts.receipt_id
              WHERE hba1c.receipt_id = ?";
$testStmt = $conn->prepare($testQuery);
$testStmt->bind_param("i", $receipt_id);
$testStmt->execute();
$testResult = $testStmt->get_result();
if ($testResult->num_rows === 0) {
    echo "<h3>No HbA1c result found for this receipt.</h3>";
    exit;
}
...
<p style="margin-bottom: 0px;margin-top: 0px;">Glycosylated Hemoglobin (HbA1c)</p>
...
<table class="result-table" style="margin-top: -15px; margin-bottom: 10px; font-size:12px; width: 100%; border-collapse: collapse;">
  <tr>
    ...
    <th style="border: none;"width: 35%;">
      <div class="chart-wrapper" id="hba1cChart">
  ...
  <?= $row['hba1c'] ?><sub style="font-size: 16px; vertical-align: sub;">mg/dL</sub>
	  </div>
	  <div style="font-size: 18px; font-weight: bold; margin-left: 40px;">
  <?php
    if ($row['hba1c'] < 5.7) {
        echo 'Normal';
    } elseif ($row['hba1c'] <= 6.4) {
        echo 'Pre-Diabetes';
    } else {
        echo 'Diabetes';
    }
  ...
  <tr>
    ...
    <td colspan="2" style="border: none; font-size: 10px;">
      <b>• Normal (&lt;5.7%) • Pre-Diabetes (5.7 - 6.4%) • Diabetes (&gt;6.4%)</b>
    </td>
  </tr>
</table>
...
<script>
const hba1cValue = parseFloat("<?= $row['hba1c'] ?>");
const chart = document.getElementById('hba1cChart');

const min = 6.0;
const max = 9.0;

if (!isNaN(hba1cValue)) {
  let percentage = 100 - ((hba1cValue - min) / (max - min)) * 100;
  ...
  marker.innerText = hba1cValue.toFixed(1);
