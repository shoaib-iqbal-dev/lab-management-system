...
// Validate input
if (empty($receipt_id) || empty($mr_no) || strtolower($test_name) !== 'serum vitamin d') {
...
$testQuery = "SELECT serum_vitamin_d.*, receipts.receipt_no, receipts.actual_fee, receipts.discount, receipts.total_fee, receipts.payment_date
              FROM serum_vitamin_d
              LEFT JOIN receipts ON serum_vitamin_d.receipt_id = receipts.receipt_id
              WHERE serum_vitamin_d.receipt_id = ?";
$testStmt = $conn->prepare($testQuery);
$testStmt->bind_param("i", $receipt_id);
$testStmt->execute();
$testResult = $testStmt->get_result();
if ($testResult->num_rows === 0) {
    echo "<h3>No Serum Vitamin D result found for this receipt.</h3>";
    exit;
}
...
<html>
<head>
    <title>Serum Vitamin D Report</title>
    <style>
        body {
...
<table class="result-table" style="margin-top: 0px; margin-bottom: 10px; font-size:12px; width: 100%; border-collapse: collapse;">
  <tr>
    <th style="width: 37%; border: none; vertical-align: top; text-align: left;padding-right: 50px;">Serum 25-OH Vitamin D<br><br><p style="font-size:7px; font-weight: normal;"></p></th>

    <th style="border: none;"width: 35%;">
      <div class="chart-wrapper" id="serum_vitamin_dChart">
  ...
  <?= $row['serum_vitamin_d'] ?><sub style="font-size: 16px; vertical-align: sub;">ng/mL</sub>
	  </div><div style="font-size: 18px; font-weight: bold; margin-left: 15px;">
  <?php
    if ($row['serum_vitamin_d'] < 12.0) {
        echo 'Severe Deficiency';
    } elseif ($row['serum_vitamin_d'] <= 20) {
        echo 'Mid Deficiency';
    } elseif ($row['serum_vitamin_d'] <= 100) {
        echo 'Optimal';
    } else {
        echo 'Excess';
    }
  ...
  <tr>
    ...
    <td colspan="3" style="border: none; font-size: 10px; text-align:center;">
      <b>• Severe Deficiency (<12.0) • Mid Deficiency (12 - 20) • Optimal (21 - 100) • Excess (>100)</b>
    </td>
  </tr>
</table>
...
<script>
const serum_vitamin_dValue = parseFloat("<?= $row['serum_vitamin_d'] ?>");
const chart = document.getElementById('serum_vitamin_dChart');

const min = 16;
const max = 24;

if (!isNaN(serum_vitamin_dValue)) {
  let percentage = 100 - ((serum_vitamin_dValue - min) / (max - min)) * 100;
  ...
  marker.innerText = serum_vitamin_dValue.toFixed(2);
