...
// Validate input
if (empty($receipt_id) || empty($mr_no) || strtolower($test_name) !== 'amylase') {
...
$testQuery = "SELECT amylase.*, receipts.receipt_no, receipts.actual_fee, receipts.discount, receipts.total_fee, receipts.payment_date
              FROM amylase
              LEFT JOIN receipts ON amylase.receipt_id = receipts.receipt_id
              WHERE amylase.receipt_id = ?";
$testStmt = $conn->prepare($testQuery);
$testStmt->bind_param("i", $receipt_id);
$testStmt->execute();
$testResult = $testStmt->get_result();
if ($testResult->num_rows === 0) {
    echo "<h3>No Amylase result found for this receipt.</h3>";
    exit;
}
...
<table width="100%">
  <tr>
	...
	<h3 style="margin-bottom: 0px;">Department of Chemistry</h3>
	</td>
