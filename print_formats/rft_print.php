...
// Validate input
if (empty($receipt_id) || empty($mr_no) || strtolower($test_name) !== 'rft') {
...
$testQuery = "SELECT rft.*, receipts.receipt_no, receipts.actual_fee, receipts.discount, receipts.total_fee, receipts.payment_date
              FROM rft
              LEFT JOIN receipts ON rft.receipt_id = receipts.receipt_id
              WHERE rft.receipt_id = ?";
$testStmt = $conn->prepare($testQuery);
$testStmt->bind_param("i", $receipt_id);
$testStmt->execute();
$testResult = $testStmt->get_result();
if ($testResult->num_rows === 0) {
    echo "<h3>No RFT result found for this receipt.</h3>";
    exit;
}
...
<table class="result-table" style="margin-top: 3px; margin-bottom: 10px; font-size:12px; width: 100%; border-collapse: collapse;">
  <tr>
    <th style="width: 37%; border: none; vertical-align: top; text-align: left;">Serum Creatinine</th>
    <th style="border: none;"width: 35%;">
      <div class="chart-wrapper" id="serum_creatinineChart">
        <div class="y-scale" style="height: 110px;top: -5px;">
          <div>1.500</div>
          <div></div>
		  <div>1.250</div>
        ...
        <div class="chart-lines">
          <div style="border-top-width: 0px;"></div><div></div><div></div><div></div>
        </div><!-- Marker will be inserted dynamically --></div></th>
		<th style="width: 28%; border: none; text-align: left;">
	  <div style="font-size: 55px; line-height: 1; font-weight: 300; font-family: 'Arial', sans-serif;">
  <?= $row['serum_creatinine'] ?><sub style="font-size: 16px; vertical-align: sub;">mg/dL</sub>
	  </div>
	  </th>
  </tr>
  <tr>
    ...
    <td colspan=2 style="border: none; font-size: 10px; text-align:left;">
      <b>• Male (0.7 - 1.4) &nbsp&nbsp• Female (0.6 - 1.1) &nbsp&nbsp• Child (0.5 - 1.2)</b>
    </td>
...
const serum_bunValue = parseFloat("<?= $row['serum_bun'] ?>");
const bunChart = document.getElementById('serum_bunChart');

// FIX: min should be LESS than max
const bunMin = 8.5;
const bunMax = 11.5;

if (!isNaN(serum_bunValue)) {
  let percentage = 100 - ((serum_bunValue - bunMin) / (bunMax - bunMin)) * 100;
  ...
  marker.className = 'marker';
  marker.style.top = `${percentage}%`;
  marker.innerText = serum_bunValue.toFixed(1);

  bunChart.appendChild(marker);
}


const serum_creatinineValue = parseFloat("<?= $row['serum_creatinine'] ?>");
const creatinineChart = document.getElementById('serum_creatinineChart');

// FIX: min should be LESS than max
const creatinineMin = 0.500;
const creatinineMax = 1.500;

if (!isNaN(serum_creatinineValue)) {
  let percentage = 100 - ((serum_creatinineValue - creatinineMin) / (creatinineMax - creatinineMin)) * 100;
  ...
  marker.className = 'marker';
  marker.style.top = `${percentage}%`;
  marker.innerText = serum_creatinineValue.toFixed(3);

  creatinineChart.appendChild(marker);
}


const egfrValue = parseFloat("<?= $row['egfr'] ?>");
...
if (!isNaN(egfrValue)) {
  let percentage = 100 - ((egfrValue - egfrMin) / (egfrMax - egfrMin)) * 100;
  ...
  marker.className = 'marker';
  marker.style.top = `${percentage}%`;
  marker.innerText = egfrValue.toFixed(1);

  egfrChart.appendChild(marker);
}
</script>
