...
// Validate input
if (empty($receipt_id) || empty($mr_no) || strtolower($test_name) !== 'bsr') {
...
$testQuery = "SELECT bsr.*, receipts.receipt_no, receipts.actual_fee, receipts.discount, receipts.total_fee, receipts.payment_date
              FROM bsr
              LEFT JOIN receipts ON bsr.receipt_id = receipts.receipt_id
              WHERE bsr.receipt_id = ?";
$testStmt = $conn->prepare($testQuery);
$testStmt->bind_param("i", $receipt_id);
$testStmt->execute();
$testResult = $testStmt->get_result();
if ($testResult->num_rows === 0) {
    echo "<h3>No BSR result found for this receipt.</h3>";
    exit;
}
