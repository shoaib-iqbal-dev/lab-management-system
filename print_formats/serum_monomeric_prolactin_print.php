...
// Validate input
if (empty($receipt_id) || empty($mr_no) || strtolower($test_name) !== 'serum monomeric prolactin') {
...
$testQuery = "SELECT serum_monomeric_prolactin.*, receipts.receipt_no, receipts.actual_fee, receipts.discount, receipts.total_fee, receipts.payment_date
              FROM serum_monomeric_prolactin
              LEFT JOIN receipts ON serum_monomeric_prolactin.receipt_id = receipts.receipt_id
              WHERE serum_monomeric_prolactin.receipt_id = ?";
$testStmt = $conn->prepare($testQuery);
$testStmt->bind_param("i", $receipt_id);
$testStmt->execute();
$testResult = $testStmt->get_result();
if ($testResult->num_rows === 0) {
    echo "<h3>No SERUM MONOMERIC PROLACTIN result found for this receipt.</h3>";
    exit;
}
...
<html>
<head>
    <title>Serum Monomeric Prolactin Report</title>
    <style>
        body {
...
<tr>
	...
	<tr>
	<td>Serum Monomeric Prolactin</td>
	<td>5.2 - 27.0</td>
	<td>ng/ml</td>
	<td><?= $row['serum_monomeric_prolactin'] ?></td>
	<td></td>
	</tr>
</table>
