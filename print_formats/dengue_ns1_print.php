...
// Validate input
if (empty($receipt_id) || empty($mr_no) || strtolower($test_name) !== 'dengue ns1') {
...
$testQuery = "SELECT dengue_ns1.*, receipts.receipt_no, receipts.actual_fee, receipts.discount, receipts.total_fee, receipts.payment_date
              FROM dengue_ns1
              LEFT JOIN receipts ON dengue_ns1.receipt_id = receipts.receipt_id
              WHERE dengue_ns1.receipt_id = ?";
$testStmt = $conn->prepare($testQuery);
$testStmt->bind_param("i", $receipt_id);
$testStmt->execute();
$testResult = $testStmt->get_result();
if ($testResult->num_rows === 0) {
    echo "<h3>No Dengue NS1 result found for this receipt.</h3>";
    exit;
}
...
<hr style="margin-top: 0px;margin-bottom: 0px;">
<p style="margin-bottom: 0px;margin-top: 0px;"><b>Dengue NS1</b></p>
...
<tr>
        ...
        <th style="width: 32%; border-top: none; border-bottom: none; border-right: none;"><h1><?= $row['dengue_ns1'] ?></h1></th>
    </tr>
	<tr>
...
<p style="font-size:12px;">By ICT Method.</p>
