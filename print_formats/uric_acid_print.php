...
// Validate input
if (empty($receipt_id) || empty($mr_no) || strtolower($test_name) !== 'uric acid') {
...
$testQuery = "SELECT uric_acid.*, receipts.receipt_no, receipts.actual_fee, receipts.discount, receipts.total_fee, receipts.payment_date
              FROM uric_acid
              LEFT JOIN receipts ON uric_acid.receipt_id = receipts.receipt_id
              WHERE uric_acid.receipt_id = ?";
$testStmt = $conn->prepare($testQuery);
$testStmt->bind_param("i", $receipt_id);
$testStmt->execute();
$testResult = $testStmt->get_result();
if ($testResult->num_rows === 0) {
    echo "<h3>No URIC ACID result found for this receipt.</h3>";
    exit;
}
...
<table class="result-table" style="margin-top: 3px; margin-bottom: 10px; font-size:12px; width: 100%; border-collapse: collapse;">
  <tr>
    <th style="width: 37%; border: none; vertical-align: top; text-align: left;padding-right: 50px;">Serum Uric Acid<br><br><p style="font-size:7px; font-weight: normal;"></p></th>
    <th style="border: none;"width: 35%;">
      <div class="chart-wrapper" id="uric_acidChart">
        <div class="y-scale" style="height: 115px;top: -10px;">
          <div></div>
		  <div>5.4</div>
          <div>5</div>
          <div>4.6</div>
  ...
  <?= $row['uric_acid'] ?><sub style="font-size: 16px; vertical-align: sub;">mg/dL</sub>
	  </div><div style="font-size: 18px; font-weight: bold; margin-left: 40px;">
  <?php
    if ($row['uric_acid'] < 3.4) {
        echo 'Low';
    } elseif ($row['uric_acid'] <= 7.0) {
        echo 'Normal';
    } else {
        echo 'High';
    }
  ...
  <tr>
    ...
    <td colspan="2" style="border: none; font-size: 10px;">
      <b>• Low (&lt;3.4) • Normal (3.4 - 7.0) • High (&gt;7.0)</b>
    </td>
  </tr>
</table>
...
<script>
const uric_acidValue = parseFloat("<?= $row['uric_acid'] ?>");
const chart = document.getElementById('uric_acidChart');

const min = 4.2;
const max = 5.6;

if (!isNaN(uric_acidValue)) {
  let percentage = 100 - ((uric_acidValue - min) / (max - min)) * 100;
  ...
  marker.innerText = uric_acidValue.toFixed(1);
