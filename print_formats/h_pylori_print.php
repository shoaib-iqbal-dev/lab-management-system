...
// Validate input
if (empty($receipt_id) || empty($mr_no) || strtolower($test_name) !== 'h pylori') {
...
$testQuery = "SELECT h_pylori.*, receipts.receipt_no, receipts.actual_fee, receipts.discount, receipts.total_fee, receipts.payment_date
              FROM h_pylori
              LEFT JOIN receipts ON h_pylori.receipt_id = receipts.receipt_id
              WHERE h_pylori.receipt_id = ?";
$testStmt = $conn->prepare($testQuery);
$testStmt->bind_param("i", $receipt_id);
$testStmt->execute();
$testResult = $testStmt->get_result();
if ($testResult->num_rows === 0) {
    echo "<h3>No H Pylori result found for this receipt.</h3>";
    exit;
}
...
<hr style="margin-top: 0px;margin-bottom: 0px;">
<p style="margin-bottom: 0px;margin-top: 0px;"><b>H Pylori</b></p>
...
<tr>
        ...
        <th style="width: 32%; border-top: none; border-bottom: none; border-right: none;"><h1><?= $row['h_pylori'] ?></h1></th>
    </tr>
	<tr>
