...
// Validate input
if (empty($receipt_id) || empty($mr_no) || strtolower($test_name) !== 'serum ferritin') {
...
$testQuery = "SELECT serum_ferritin.*, receipts.receipt_no, receipts.actual_fee, receipts.discount, receipts.total_fee, receipts.payment_date
              FROM serum_ferritin
              LEFT JOIN receipts ON serum_ferritin.receipt_id = receipts.receipt_id
              WHERE serum_ferritin.receipt_id = ?";
$testStmt = $conn->prepare($testQuery);
$testStmt->bind_param("i", $receipt_id);
$testStmt->execute();
$testResult = $testStmt->get_result();
if ($testResult->num_rows === 0) {
    echo "<h3>No SERUM FERRITIN result found for this receipt.</h3>";
    exit;
}
...
<html>
<head>
    <title>Serum Ferritin Report</title>
    <style>
        body {
...
<tr>
	...
	<td>(See Below)</td>
	<td>ng/mL</td>
	<td><?= $row['serum_ferritin'] ?></td>
	<td></td>
	</tr>
</table>

<p style="font-size:12px;"><u>Reference Values:</u><br><p>
