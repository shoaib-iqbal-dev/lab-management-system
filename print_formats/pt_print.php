...
// Validate input
if (empty($receipt_id) || empty($mr_no) || strtolower($test_name) !== 'pt') {
...
$testQuery = "SELECT pt.*, receipts.receipt_no, receipts.actual_fee, receipts.discount, receipts.total_fee, receipts.payment_date
              FROM pt
              LEFT JOIN receipts ON pt.receipt_id = receipts.receipt_id
              WHERE pt.receipt_id = ?";
...
if ($testResult->num_rows === 0) {
    echo "<h3>No PT result found for this receipt.</h3>";
    exit;
}
