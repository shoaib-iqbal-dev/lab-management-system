...
// Validate input
if (empty($receipt_id) || empty($mr_no) || strtolower($test_name) !== 'serum prolactin') {
...
$testQuery = "SELECT serum_prolactin.*, receipts.receipt_no, receipts.actual_fee, receipts.discount, receipts.total_fee, receipts.payment_date
              FROM serum_prolactin
              LEFT JOIN receipts ON serum_prolactin.receipt_id = receipts.receipt_id
              WHERE serum_prolactin.receipt_id = ?";
$testStmt = $conn->prepare($testQuery);
$testStmt->bind_param("i", $receipt_id);
$testStmt->execute();
$testResult = $testStmt->get_result();
if ($testResult->num_rows === 0) {
    echo "<h3>No SERUM PROLACTIN result found for this receipt.</h3>";
    exit;
}
...
<html>
<head>
    <title>Serum Prolactin Report</title>
    <style>
        body {
