...
// Validate input
if (empty($receipt_id) || empty($mr_no) || strtolower($test_name) !== 'upt') {
...
$testQuery = "SELECT upt.*, receipts.receipt_no, receipts.actual_fee, receipts.discount, receipts.total_fee, receipts.payment_date
              FROM upt
              LEFT JOIN receipts ON upt.receipt_id = receipts.receipt_id
              WHERE upt.receipt_id = ?";
$testStmt = $conn->prepare($testQuery);
$testStmt->bind_param("i", $receipt_id);
$testStmt->execute();
$testResult = $testStmt->get_result();
if ($testResult->num_rows === 0) {
    echo "<h3>No UPT result found for this receipt.</h3>";
    exit;
}
...
<table width="100%">
  <tr>
	...
	<h3 style="margin-bottom: 0px;">Department of Pathalogy</h3>
	</td>
...
<hr style="margin-top: 0px;margin-bottom: 0px;">
<p style="margin-bottom: 0px;margin-top: 0px;"><b>Urine Pregnancy Test</b></p>
...
<tr>
        ...
        <th style="width: 32%; border-top: none; border-bottom: none; border-right: none;"><h1><?= $row['upt'] ?></h1></th>
    </tr>
	<tr>
