...
if ($receipt_id <= 0) {
    ...
    $check_receipt_sql = "SELECT * FROM serum_ferritin WHERE receipt_id = '$receipt_id'";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // Get data from form
    $serum_ferritin = $_POST['serum_ferritin'];
    

    if ($edit_mode) {
        // Update existing record
        $update_sql = "UPDATE serum_ferritin SET
        ...
        // Insert new record
        $insert_sql = "INSERT INTO serum_ferritin (
                        receipt_id, serum_ferritin, reporting_datetime
                    ) VALUES (
                        '$receipt_id', '$serum_ferritin', NOW()
                    )";
...
<head>
        <meta charset="UTF-8">
    <title>Srum Ferritin Test Result</title>
...
<div class="container" style="margin-bottom: 100px;">
    <h2><?= $edit_mode ? "Edit Srum Ferritin Test Result" : "Enter Srum Ferritin Test Result" ?></h2>
    ...
    <form method="post" action="">
        <label for="serum_ferritin">Srum Ferritin:</label>
        <input type="text" name="serum_ferritin" id="serum_ferritin" required value="<?= htmlspecialchars($existing_data['serum_ferritin']) ?>" />
