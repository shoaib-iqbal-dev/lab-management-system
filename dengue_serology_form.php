...
if ($receipt_id <= 0) {
    ...
    $check_sql = "SELECT * FROM dengue_serology WHERE receipt_id = $receipt_id";
    ...
    if (mysqli_num_rows($check_result) > 0) {
        ...
        $existing_data = ['igg' => '','igm' => '', 'reporting_datetime' => ''];
    }
}

if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $igg = mysqli_real_escape_string($conn, $_POST['igg']);
    $igm = mysqli_real_escape_string($conn, $_POST['igm']);

    if ($edit_mode) {
        $update_sql = "UPDATE dengue_serology SET igg = '$igg',igm = '$igm', reporting_datetime = NOW() WHERE receipt_id = $receipt_id";
        ...
        $insert_sql = "INSERT INTO dengue_serology (receipt_id, igg, igm, reporting_datetime) VALUES ($receipt_id, '$igg', '$igm', NOW())";
...
<title>DENGUE SEROLOGY Test Form</title>
<style>
        body {
...
<div class="container" style="margin-bottom: 100px;">
    ...
    <form method="post" action="">
        <label for="igm">Dengue IGG Result:</label>
        <input type="text" name="igm" id="igm" required value="<?= htmlspecialchars($existing_data['igm']) ?>" />


        <label for="igg">Dengue IGM Result:</label>
        <input type="text" name="igg" id="igg" required value="<?= htmlspecialchars($existing_data['igg']) ?>" />
