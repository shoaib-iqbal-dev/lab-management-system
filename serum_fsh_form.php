...
if ($receipt_id <= 0) {
    ...
    $check_receipt_sql = "SELECT * FROM serum_fsh WHERE receipt_id = '$receipt_id'";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // Get data from form
    $serum_fsh = $_POST['serum_fsh'];
    

    if ($edit_mode) {
        // Update existing record
        $update_sql = "UPDATE serum_fsh SET
                        serum_fsh = '$serum_fsh',
        ...
        // Insert new record
        $insert_sql = "INSERT INTO serum_fsh (
                        receipt_id, serum_fsh, reporting_datetime
                    ) VALUES (
                        '$receipt_id', '$serum_fsh', NOW()
                    )";
...
<head>
        <meta charset="UTF-8">
    <title>Serum FSH Test Result</title>
...
<div class="container" style="margin-bottom: 100px;">
    <h2><?= $edit_mode ? "Edit Serum FSH Test Result" : "Enter Serum FSH Test Result" ?></h2>
    ...
    <form method="post" action="">
        <label for="serum_fsh">Serum FSH:</label>
        <input type="text" name="serum_fsh" id="serum_fsh" required value="<?= htmlspecialchars($existing_data['serum_fsh']) ?>" />
