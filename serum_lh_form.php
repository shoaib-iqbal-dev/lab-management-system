...
if ($receipt_id <= 0) {
    ...
    $check_receipt_sql = "SELECT * FROM serum_lh WHERE receipt_id = '$receipt_id'";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // Get data from form
    $serum_lh = $_POST['serum_lh'];
    

    if ($edit_mode) {
        // Update existing record
        $update_sql = "UPDATE serum_lh SET
                        serum_lh = '$serum_lh',
        ...
        // Insert new record
        $insert_sql = "INSERT INTO serum_lh (
                        receipt_id, serum_lh, reporting_datetime
                    ) VALUES (
                        '$receipt_id', '$serum_lh', NOW()
                    )";
...
<head>
        <meta charset="UTF-8">
    <title>Serum Lh Test Result</title>
...
<div class="container" style="margin-bottom: 100px;">
    <h2><?= $edit_mode ? "Edit Serum Lh Test Result" : "Enter Serum Lh Test Result" ?></h2>
    ...
    <form method="post" action="">
        <label for="serum_lh">Serum Lh:</label>
        <input type="text" name="serum_lh" id="serum_lh" required value="<?= htmlspecialchars($existing_data['serum_lh']) ?>" />
