...
if ($receipt_id <= 0) {
    ...
    $check_receipt_sql = "SELECT * FROM serum_amh WHERE receipt_id = '$receipt_id'";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // Get data from form
    $serum_amh = $_POST['serum_amh'];
    

    if ($edit_mode) {
        // Update existing record
        $update_sql = "UPDATE serum_amh SET
                        serum_amh = '$serum_amh',
        ...
        // Insert new record
        $insert_sql = "INSERT INTO serum_amh (
                        receipt_id, serum_amh, reporting_datetime
                    ) VALUES (
                        '$receipt_id', '$serum_amh', NOW()
                    )";
...
<head>
        <meta charset="UTF-8">
    <title>Serum AMH Test Result</title>
...
<div class="container" style="margin-bottom: 100px;">
    <h2><?= $edit_mode ? "Edit Serum AMH Test Result" : "Enter Serum AMH Test Result" ?></h2>
    ...
    <form method="post" action="">
        <label for="serum_amh">Serum AMH:</label>
        <input type="text" name="serum_amh" id="serum_amh" required value="<?= htmlspecialchars($existing_data['serum_amh']) ?>" />
