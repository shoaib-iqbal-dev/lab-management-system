...
if ($receipt_id <= 0) {
    ...
    $check_sql = "SELECT * FROM stool_h_pylori WHERE receipt_id = $receipt_id";
    ...
    if (mysqli_num_rows($check_result) > 0) {
        ...
        $existing_data = ['stool_h_pylori' => '', 'reporting_datetime' => ''];
    }
}

if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $stool_h_pylori = mysqli_real_escape_string($conn, $_POST['stool_h_pylori']);

    if ($edit_mode) {
        $update_sql = "UPDATE stool_h_pylori SET stool_h_pylori = '$stool_h_pylori', reporting_datetime = NOW() WHERE receipt_id = $receipt_id";
        ...
        $insert_sql = "INSERT INTO stool_h_pylori (receipt_id, stool_h_pylori, reporting_datetime) VALUES ($receipt_id, '$stool_h_pylori', NOW())";
...
<title>STOOL H PYLORI Test Form</title>
<style>
        body {
...
<div class="container" style="margin-bottom: 100px;">
    ...
    <form method="post" action="">
        <label for="stool_h_pylori">Stool H Pylori Result:</label>
        <input type="text" name="stool_h_pylori" id="stool_h_pylori" required value="<?= htmlspecialchars($existing_data['stool_h_pylori']) ?>" />
